"""Tests for desloppify.detectors.coupling — coupling analysis detectors."""
...
from desloppify.detectors.coupling import (
...
class TestDetectCouplingViolations:
    ...
    def test_counts_shared_to_shared_edges(self):
        ...
        assert entries == []
        assert total_edges == 1  # shared->shared edge counted
    ...
    def test_returns_entries_and_total_edges(self):
        ...
        assert len(entries) == 1
        assert total_edges == 2  # one shared->tools + one shared->shared
    ...
    def test_sorted_by_file_then_target(self):
        ...
        targets_for_a = [e["target"] for e in entries if e["file"] == f"{SHARED_PREFIX}a.ts"]
...
class TestDetectBoundaryCandidates:
    ...
    def test_returns_entries_and_total_shared(self, tmp_path):
        ...
        graph = {
            ...
            str(f2): _graph_entry(importer_count=2, importers=[f"{tp}x/b.ts", f"{tp}y/c.ts"]),
...
class TestDetectCrossToolImports:
    ...
    def test_same_tool_imports_counted_as_edges_not_violations(self):
        ...
        assert entries == []
        assert total_edges == 1  # edge counted, but not a violation
    ...
    def test_returns_entries_and_total_edges(self):
        ...
        assert total_edges == 2  # both edges counted

...
import desloppify.app.commands.review.cmd as review_cmd_mod
from desloppify import utils as _utils_mod
from desloppify.cli import create_parser
from desloppify.app.commands.review.cmd import (
    _do_import,
    _do_prepare,
    _do_run_batches,
    _setup_lang,
...
from desloppify.intelligence.review.context import _serialize_context
from desloppify.intelligence.review.importing.findings import _update_review_cache
from desloppify.intelligence.review.selection import _count_fresh, _count_stale
...
from desloppify.utils import disable_file_cache, enable_file_cache
...
class TestImportReviewFindings:
    ...
    def test_import_updates_review_cache(
        ...
        with patch("desloppify.intelligence.review.importing.findings.PROJECT_ROOT", tmp_path):
    ...
    def test_import_merges_with_state(self, state_with_findings, sample_findings_data):
        diff = import_review_findings(
            sample_findings_data, state_with_findings, "typescript"
        )
...
class TestScoringIntegration:
    ...
    def test_review_findings_not_auto_resolved_by_scan(
        ...
        merge_scan(
            empty_state, [], lang="typescript", potentials={"unused": 10, "smells": 50}
        )
...
class TestCmdReviewPrepare:
    def test_do_prepare_writes_query_json(
        ...
        with (
            patch(
                "desloppify.app.commands.review.cmd._setup_lang",
                return_value=(mock_lang_with_zones, file_list),
            ),
            patch("desloppify.app.commands.review.cmd._write_query", capture_query),
        ):
            _do_prepare(
                args,
                empty_state,
    ...
    def test_do_prepare_prints_narrative_reminders(self, mock_lang_with_zones, empty_state, tmp_path, capsys):
        ...
        from desloppify.app.commands.review.cmd import _do_prepare
        ...
        with patch("desloppify.app.commands.review.cmd._setup_lang", return_value=file_list), \
             patch("desloppify.app.commands._helpers._write_query", lambda _data: None), \
    ...
    def test_do_import_blocks_large_assessment_swing_without_findings(self, empty_state, tmp_path):
        from desloppify.app.commands.review.cmd import _do_import
    ...
    def test_do_run_batches_dry_run_generates_packet_and_prompts(
        ...
        with (
            ...
            patch(
                "desloppify.app.commands.review.cmd.review_mod.prepare_holistic_review",
                return_value=prepared,
            ),
            patch(
                "desloppify.app.commands.review.cmd._write_query",
            ),
            patch(
                "desloppify.app.commands.review.cmd.PROJECT_ROOT",
                tmp_path,
            ),
            patch(
                "desloppify.app.commands.review.cmd.REVIEW_PACKET_DIR",
                review_packet_dir,
            ),
            patch(
                "desloppify.app.commands.review.cmd.SUBAGENT_RUNS_DIR",
                runs_dir,
            ),
            patch(
                "desloppify.app.commands.review.cmd._do_import",
            ) as mock_import,
        ):
            _do_run_batches(
    ...
    def test_do_run_batches_merges_outputs_and_imports(self, empty_state, tmp_path):
        ...
        with (
            patch(
                "desloppify.app.commands.review.cmd.subprocess.run",
    ...
    def test_run_codex_batch_returns_127_when_runner_missing(self, tmp_path):
        ...
        with patch(
            ...
            code = review_cmd_mod._run_codex_batch(
                prompt="test prompt",
                repo_root=tmp_path,
    ...
    def test_run_followup_scan_returns_nonzero_code(self, tmp_path):
        with patch(
            ...
            code = review_cmd_mod._run_followup_scan(
    ...
    def test_do_run_batches_scan_after_import_exits_on_failed_followup(
        ...
        with (
            ...
            patch(
                "desloppify.app.commands.review.cmd.runner_helpers_mod.execute_batches",
                return_value=[],
            ),
            patch(
                "desloppify.app.commands.review.cmd.runner_helpers_mod.collect_batch_results",
                return_value=([{"assessments": {}, "dimension_notes": {}, "findings": []}], []),
            ),
            patch(
                "desloppify.app.commands.review.cmd._merge_batch_results",
                return_value={"assessments": {}, "dimension_notes": {}, "findings": []},
            ),
            patch(
                "desloppify.app.commands.review.cmd._run_followup_scan",
                return_value=7,
            ),
        ):

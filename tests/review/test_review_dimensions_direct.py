...
def test_per_file_dimensions_schema_validation_rejects_bad_prompt_shape(monkeypatch):
    ...
    dimensions_data_mod.load_per_file_dimensions.cache_clear()
    try:
        with pytest.raises(ValueError, match="look_for"):
            dimensions_data_mod.load_per_file_dimensions()
    finally:
        dimensions_data_mod.load_per_file_dimensions.cache_clear()


def test_holistic_dimensions_validation_requires_prompt_for_each_dimension(monkeypatch):
    def _bad_payload(_filename: str) -> dict:
        return {
            "holistic_dimensions": ["cross_module_architecture", "error_consistency"],
            "dimension_prompts": {
    ...
    monkeypatch.setattr(dimensions_data_mod, "_load_json_payload", _bad_payload)
    dimensions_data_mod.load_holistic_dimensions.cache_clear()
    try:
        with pytest.raises(ValueError, match="missing prompts"):
            dimensions_data_mod.load_holistic_dimensions()
    finally:
        dimensions_data_mod.load_holistic_dimensions.cache_clear()
...
def test_load_per_file_dimensions_for_lang_prefers_language_payload(
    ...
    (shared_dir / "per_file_dimensions.json").write_text(json.dumps(shared_payload))
    (lang_dir / "python" / "review_data" / "per_file_dimensions.json").write_text(
        json.dumps(lang_payload)
    )

    monkeypatch.setattr(dimensions_data_mod, "_DATA_DIR", shared_dir)
    monkeypatch.setattr(dimensions_data_mod, "_LANG_DIR", lang_dir)
    dimensions_data_mod.load_per_file_dimensions_for_lang.cache_clear()
    dimensions_data_mod.load_per_file_dimensions.cache_clear()
    try:
        dims, prompts, _ = dimensions_data_mod.load_per_file_dimensions_for_lang(
            "python"
        )
    finally:
        dimensions_data_mod.load_per_file_dimensions_for_lang.cache_clear()
...
def test_load_holistic_dimensions_for_lang_falls_back_to_shared(tmp_path, monkeypatch):
    shared_dir = tmp_path / "review_data"
    lang_dir = tmp_path / "lang_data"
    shared_dir.mkdir(parents=True)
    ...
    shared_payload = {
        "holistic_dimensions": ["shared_dim"],
        "dimension_prompts": {
            "shared_dim": {
                "description": "shared desc",
                "look_for": ["x"],
    ...
    (shared_dir / "holistic_dimensions.json").write_text(json.dumps(shared_payload))

    monkeypatch.setattr(dimensions_data_mod, "_DATA_DIR", shared_dir)
    monkeypatch.setattr(dimensions_data_mod, "_LANG_DIR", lang_dir)
    dimensions_data_mod.load_holistic_dimensions_for_lang.cache_clear()
    dimensions_data_mod.load_holistic_dimensions.cache_clear()
    try:
        dims, prompts, _ = dimensions_data_mod.load_holistic_dimensions_for_lang(
            "python"
        )
    finally:
        dimensions_data_mod.load_holistic_dimensions_for_lang.cache_clear()
...
def test_load_per_file_dimensions_for_lang_applies_override_patch(
    tmp_path, monkeypatch
):
    shared_dir = tmp_path / "review_data"
    lang_dir = tmp_path / "lang_data"
    ...
    (shared_dir / "per_file_dimensions.json").write_text(json.dumps(shared_payload))
    (
        lang_dir / "python" / "review_data" / "per_file_dimensions.override.json"
    ...
    try:
        dims, prompts, system_prompt = (
            dimensions_data_mod.load_per_file_dimensions_for_lang("python")
        )
    finally:
        dimensions_data_mod.load_per_file_dimensions_for_lang.cache_clear()
        dimensions_data_mod.load_per_file_dimensions.cache_clear()
...
def test_load_holistic_dimensions_for_lang_applies_override_patch(
    tmp_path, monkeypatch
):
    shared_dir = tmp_path / "review_data"
    lang_dir = tmp_path / "lang_data"
    ...
    override_payload = {
        "holistic_dimensions_remove": ["b"],
        "holistic_dimensions_append": ["c"],
        "dimension_prompts": {
    ...
    (shared_dir / "holistic_dimensions.json").write_text(json.dumps(shared_payload))
    (
        lang_dir / "python" / "review_data" / "holistic_dimensions.override.json"
    ).write_text(json.dumps(override_payload))
...
def test_parse_per_file_payload_supports_meta_enabled_defaults():
    payload = {
        "dimension_prompts": {
            "structure_signal": {
    ...
    dims, prompts, _ = dimensions_validation_mod.parse_per_file_payload(
...
def test_load_per_file_dimensions_for_lang_meta_enabled_dimension_requires_no_append(
    tmp_path, monkeypatch
):
    shared_dir = tmp_path / "review_data"
    lang_dir = tmp_path / "lang_data"
...
def test_dimension_metadata_can_override_weight_per_language(monkeypatch):
    ...
    lang_h_payload = ([], {}, "h" * 120)

    monkeypatch.setattr(
        dimensions_metadata_mod, "load_per_file_dimensions", lambda: shared_per_payload
    )
    monkeypatch.setattr(
        dimensions_metadata_mod, "load_holistic_dimensions", lambda: shared_h_payload
    )
    monkeypatch.setattr(
        dimensions_metadata_mod,
        "load_per_file_dimensions_for_lang",
        lambda _lang: lang_per_payload,
    )
    monkeypatch.setattr(
        dimensions_metadata_mod,
        "load_holistic_dimensions_for_lang",
        lambda _lang: lang_h_payload,
    )
    monkeypatch.setattr(dimensions_metadata_mod, "_available_languages", lambda: [])
    dimensions_metadata_mod.load_subjective_dimension_metadata.cache_clear()

...
from desloppify.intelligence.review.context import _extract_imported_names, _file_excerpt
from desloppify.intelligence.review.prepare import _HOLISTIC_WORKFLOW
...
class TestHolisticConstants:
    def test_fifteen_dimensions(self):
        assert len(HOLISTIC_DIMENSIONS) == 15
...
class TestSiblingBehavior:
    def test_detects_outlier(self, tmp_path):
        ...
        files = [
            ...
            for n in [f"cmd_{i}.py" for i in range(3)] + ["cmd.py"]
        ]
...
class TestNewInvestigationBatches:
    ...
    def test_governance_batch_generated_when_reference_files_exist(self, tmp_path):
        ...
        assert "scripts/checks.sh" in governance["files_to_read"]

...
def _gather_ai_debt_signals(file_contents: dict[str, str]) -> dict:
    return gather_ai_debt_signals(file_contents, rel_fn=_test_rel)


def _gather_auth_context(file_contents: dict[str, str]) -> dict:
    return gather_auth_context(file_contents, rel_fn=_test_rel)


def _gather_migration_signals(file_contents: dict[str, str], lang) -> dict:
    return gather_migration_signals(file_contents, lang, rel_fn=_test_rel)


def _classify_error_strategy(content: str) -> str | None:
    return classify_error_strategy(content)
...
class TestBuildHolisticContext:
    ...
    def test_authorization_section_present_with_routes(self, mock_lang, empty_state):
        ...
        with patch(
            "desloppify.intelligence.review.context_holistic.read_file_text", return_value=content
        ):
    ...
    def test_ai_debt_section_present_with_signals(self, mock_lang, empty_state):
        ...
        with patch(
            "desloppify.intelligence.review.context_holistic.read_file_text",

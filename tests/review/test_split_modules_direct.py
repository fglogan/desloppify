...
from desloppify.intelligence.review.importing.holistic import (
    _extract_findings_and_assessments as extract_holistic,
)
from desloppify.intelligence.review.importing.per_file import (
    _extract_findings_and_assessments as extract_per_file,
)
from desloppify.intelligence.review.importing.shared import (
    extract_reviewed_files,
    store_assessments,
)
from desloppify.intelligence.review.prepare_internal.remediation_engine import empty_plan


def test_import_split_extract_helpers_accept_list_and_object_payloads():
    findings, assessments = extract_per_file([{"summary": "x"}])
    assert findings == [{"summary": "x"}]
    assert assessments is None

    findings2, assessments2, reviewed_files = extract_holistic(
        {
            "findings": [{"summary": "y"}],

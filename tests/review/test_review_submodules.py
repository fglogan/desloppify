...
from desloppify.intelligence.review.importing.findings import (
    _extract_findings_and_assessments,
    _extract_reviewed_files,
    _store_assessments,
...
from desloppify.intelligence.review.remediation import _empty_plan, generate_remediation_plan
...
class TestBuildFileRequests:
    def test_basic(self, mock_lang, empty_state):
        with (
            ...
            patch("desloppify.intelligence.review.prepare._abs", side_effect=lambda x: x),
        ):
...
class TestPrepareReview:
    def test_returns_expected_keys(self, mock_lang, empty_state):
        with (
            ...
            patch("desloppify.intelligence.review.prepare._serialize_context", return_value={}),
            patch("desloppify.intelligence.review.prepare._count_fresh", return_value=0),
            patch("desloppify.intelligence.review.prepare._count_stale", return_value=0),
        ):
...
class TestExtractReviewedFiles:
    def test_non_dict_payload(self):
        assert _extract_reviewed_files([]) == []
    ...
    def test_valid_reviewed_files_dedupes_and_filters(self):
        ...
        assert _extract_reviewed_files(payload) == ["src/a.ts", "src/b.ts"]
...
class TestStoreAssessments:
    ...
    def test_dict_value_format(self, empty_state):
        _store_assessments(
            empty_state, {"naming": {"score": 75, "extra": "data"}}, "per_file"
        )
...
class TestImportReviewFindings:
    ...
    def test_normalizes_invalid_confidence(self, empty_state):
        ...
        _ = import_review_findings(data, empty_state, "typescript")
...
class TestUpdateReviewCache:
    def test_updates_cache(self, empty_state):
        with (
            patch("desloppify.intelligence.review.importing.findings.PROJECT_ROOT", Path("/fake")),
            patch(
                "desloppify.intelligence.review.importing.findings.utc_now",
...
class TestUpdateHolisticReviewCache:
    def test_updates_holistic_cache(self, empty_state):
        with patch(
            "desloppify.intelligence.review.importing.findings.utc_now", return_value="2026-02-01"
        ):
    ...
    def test_uses_codebase_metrics_total_files_when_present(self, empty_state):
        ...
        with patch(
            ...
            _update_holistic_review_cache(empty_state, [], lang_name="python")

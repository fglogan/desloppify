...
"""
    names = patterns_mod._extract_imported_names(content)
    assert "Foo" in names
    assert "Bar" in names
    assert "one" in names
    assert "two" in names
...
__all__ = ["py_func"]
"""
    tags = patterns_mod._default_review_module_patterns(content)
...
def test_compiled_regexes_match_expected_symbols():
    assert patterns_mod._FUNC_NAME_RE.search("def compute(x):").group(1) == "compute"
    assert patterns_mod._CLASS_NAME_RE.search("class Handler:").group(1) == "Handler"
    assert patterns_mod._ERROR_PATTERNS["throws"].search("raise ValueError('bad')")
    assert patterns_mod._NAME_PREFIX_RE.search("compute_total")

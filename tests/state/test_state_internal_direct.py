...
import desloppify.engine.state_internal.migrations as migrations_mod
...
def test_migrate_progress_scores_promotes_legacy_keys():
    state = {
        "score": 77.0,
        "objective_strict": 75.0,
        "scan_history": [{"score": 70.0, "objective_strict": 69.0}],
    }

    migrations_mod._migrate_progress_scores(state)

    assert state["objective_score"] == 77.0
    assert state["overall_score"] == 77.0
    assert state["strict_score"] == 75.0
    assert state["verified_strict_score"] == 75.0
    assert "score" not in state
    assert "objective_strict" not in state
    assert state["scan_history"][0]["objective_score"] == 70.0
    assert state["scan_history"][0]["overall_score"] == 70.0
    assert state["scan_history"][0]["strict_score"] == 69.0
    assert state["scan_history"][0]["verified_strict_score"] == 69.0


def test_migrate_progress_scores_promotes_subjective_integrity_legacy_keys():
    state = {
        "subjective_integrity_status": "warn",
        "scan_history": [{"subjective_integrity_status": "penalized"}],
    }

    migrations_mod._migrate_progress_scores(state)

    assert state["subjective_integrity"]["status"] == "warn"
    assert state["scan_history"][0]["subjective_integrity"]["status"] == "penalized"
    assert "subjective_integrity_status" not in state
    assert "subjective_integrity_status" not in state["scan_history"][0]
...
def test_load_state_missing_and_backup_fallback(tmp_path):
    ...
    backup.write_text(json.dumps(schema_mod._empty_state()))

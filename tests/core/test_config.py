...
class TestLoadSaveConfig:
    ...
    def test_invalid_badge_path_in_file_resets_to_default(self, tmp_path):
        p = tmp_path / "config.json"
        p.parent.mkdir(parents=True, exist_ok=True)
        p.write_text(json.dumps({"badge_path": "scorecard.png"}))
        cfg = load_config(p)
        assert cfg["badge_path"] == "assets/scorecard.png"
...
class TestSetConfigValue:
    ...
    def test_set_badge_path_without_directory_raises(self):
        cfg = default_config()
        with pytest.raises(ValueError, match="directory \\+ filename"):

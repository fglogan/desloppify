...
from desloppify.app.commands.helpers.query import _write_query
...
class TestCliSmokeBaseline:
    def test_smoke_snapshot_script_exists(self):
        script = Path("scripts/cli_smoke_snapshot.sh")
        assert script.exists()
        assert script.read_text().startswith("#!/usr/bin/env bash")

    def test_smoke_check_script_exists(self):
        script = Path("scripts/cli_smoke_check.sh")
        assert script.exists()
        assert script.read_text().startswith("#!/usr/bin/env bash")

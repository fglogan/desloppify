...
import desloppify.engine.state_internal.migrations as migrations
...
def test_direct_module_coverage_smoke_signals():
    ...
    assert callable(status_render._show_tier_progress_table)
    ...
    assert isinstance(runtime_state.EXCLUSION_CONFIG.values, tuple)
    assert isinstance(runtime_state.SOURCE_FILE_CACHE.max_entries, int)
    runtime_state.CACHE_ENABLED.set(True)
    assert bool(runtime_state.CACHE_ENABLED)
    runtime_state.CACHE_ENABLED.set(False)
    ...
    # state internals
    assert callable(migrations._migrate_progress_scores)
    ...
    assert callable(scorecard_left_panel._draw_left_panel)
    ...
    assert callable(review_dimensions_validation.parse_per_file_payload)

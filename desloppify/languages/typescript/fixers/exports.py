    ...
    def transform(
        ...
        for line_idx in range(len(lines)):
            ...
            if m:
                ...
                name = next(
                    (e["name"] for e in file_entries if e["line"] == lineno), "?"
                )
            ...
            if stripped.startswith("export {"):
                ...
                if brace_content:
                    all_names = [
                        n.strip()
                        for n in brace_content.group(1).split(",")
                        if n.strip()
                    ]
                    remaining = [
                        n
                        for n in all_names
                        if n.split(" as ")[0].strip() not in names_in_entry
                    ]
                    if not remaining:
                        lines[line_idx] = ""
                    else:
                        ...
                        if from_match:
                            ...
                            from_clause = ";"
                        lines[line_idx] = (
                            f"{indent_str}export {{ {', '.join(remaining)} }}{from_clause}\n"
                        )

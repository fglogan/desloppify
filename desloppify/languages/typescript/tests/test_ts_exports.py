"""Tests for desloppify.languages.typescript.detectors.exports — dead export detection."""
...
from desloppify.languages.typescript.detectors.exports import (
    EXPORT_DECL_RE,
    detect_dead_exports,
)
...
def _set_project_root(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(exports_detector_mod, "SRC_PATH", tmp_path)
...
class TestDetectDeadExports:
    ...
    def test_export_used_by_multiple_importers(self, tmp_path):
        ...
        _write(
            tmp_path, "a.ts", "import { sharedUtil } from './shared';\nsharedUtil();\n"
        )
        _write(
            tmp_path, "b.ts", "import { sharedUtil } from './shared';\nsharedUtil();\n"
        )

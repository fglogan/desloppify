...
from desloppify.languages.framework.base import FixerConfig, FixResult
from desloppify.utils import colorize, rel
from desloppify.app.commands.review import cmd as review_cmd_mod
...
def _cmd_fix_review(args):
    ...
    lang_run, found_files = review_cmd_mod._setup_lang(
        lang_cfg,
        path,
        runtime.config,
    )
    data = review_mod.prepare_review(path, lang_run, state, files=found_files or None)
...
_COMMAND_POST_FIX: dict[
    str, object
] = {}  # populated after _cascade_unused_import_cleanup is defined
...
def _apply_and_report(
    ...
    narrative = narrative_mod.compute_narrative(
        state, lang=fix_lang_name, command="fix"
    )
    ...
    if len(entries) > skipped:
        print(
            colorize(
                ...
                "(complex patterns, rest elements, etc.)",
                "dim",
            )
        )
    print()
...
def _print_file_sample(result, entries):
    ...
    try:
        ...
        return
    file_entries = [
        e for e in entries if e["file"] == filepath and e.get("name", "") in removed_set
    ]
    shown = 0

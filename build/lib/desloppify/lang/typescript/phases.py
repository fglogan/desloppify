    ...
    for e in pattern_entries:
        results.append(make_finding(
            ...
            detail={"family": e["family"], "patterns_used": e["patterns_used"],
                    ...
                    "pattern_evidence": e.get("pattern_evidence", {})},
        ))
...
def _phase_smells(path: Path, lang: LangConfig) -> tuple[list[dict], dict[str, int]]:
    ...
    for e in sig_entries:
        label = f"{e['name']}()" if e.get("group_type") == "name" else e["name"]
        variance_parts = []
        if e.get("has_param_variance"):
            variance_parts.append(f"{e['signature_count']} parameter signatures")
        if e.get("has_return_variance"):
            variance_parts.append(f"{e['return_signature_count']} return signatures")
        variance_summary = " and ".join(variance_parts) if variance_parts else "inconsistent signatures"
        results.append(make_finding(
            "smells", e["files"][0], f"sig_variance::{e['name']}",
            tier=3, confidence="medium",
            summary=f"Signature variance: {label} has {variance_summary} "
                    f"across {e['file_count']} files",
            detail={"function": e["name"], "file_count": e["file_count"],
                    "signature_count": e["signature_count"],
                    "group_type": e.get("group_type", "name"),
                    "return_signature_count": e.get("return_signature_count", 0),
                    "return_signatures": e.get("return_signatures", []),

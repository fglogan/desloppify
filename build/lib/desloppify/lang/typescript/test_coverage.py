...
TRIVIAL_ASSERT_PATTERNS = [
    re.compile(p) for p in [
        r"\bexpect\s*\(\s*true\s*\)\s*\.\s*to(?:Be|Equal|StrictEqual)\s*\(\s*true\s*\)",
        r"\bexpect\s*\([^)]*\)\s*\.\s*toBeDefined\s*\(",
        r"\bexpect\s*\([^)]*\)\s*\.\s*toBeTruthy\s*\(",
    ]
]
NEGATIVE_PATH_PATTERNS = [
    re.compile(p) for p in [
        r"\btoThrow(?:Error)?\s*\(",
        r"\brejects\s*\.\s*toThrow(?:Error)?\s*\(",
        r"\bassert\.throws\s*\(",
    ]
]
...
def is_runtime_entrypoint(filepath: str, content: str) -> bool:
    """Heuristic for TS runtime entrypoints invoked outside local import graph."""
    norm = filepath.replace("\\", "/")
    if "/supabase/functions/" in norm and norm.endswith("/index.ts"):
        return True
    if "/supabase/functions/" in norm and norm.endswith("/index.tsx"):
        return True
    if re.search(r"\bserve\s*\(", content):
        # Deno/edge-style service entrypoints often use URL or jsr imports.
        if "deno.land" in content or "jsr:" in content or "Deno." in content:
            return True
    return False

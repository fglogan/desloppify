...
def _update_objective_health(state: dict, findings: dict):
    ...
    _update_ignore_integrity_warning(state)
    _migrate_progress_scores(state)


def _update_ignore_integrity_warning(state: dict) -> None:
    """Record warning metadata when ignore suppression is high."""
...
def get_strict_all_detected_score(state: dict) -> float | None:
    """Strict score interpretation including ignored + zone-excluded findings.

    This recomputes strict scoring for the current scan scope while:
    - forcing all zones into score scope ("all detected" view), and
    - adding ignored findings from the latest scan as open failures.
    """
    potentials_by_lang = state.get("potentials", {})
    if not potentials_by_lang:
        return None
    ...
    merged = merge_potentials(potentials_by_lang)
    if not merged:
        return None
    ...
    for fid, finding in scoped.items():
        copy = dict(finding)
        copy["zone"] = "production"
        all_detected[fid] = copy

    ignored = (state.get("ignore_integrity", {}) or {}).get("ignored_findings", [])
    if isinstance(ignored, list):
        for idx, finding in enumerate(ignored):
            if not isinstance(finding, dict):
                continue
            detector = finding.get("detector", "unknown")
            file = finding.get("file", ".")
            base_id = finding.get("id") or f"{detector}::{file}::ignored"
            fid = f"ignored::{base_id}::{idx}"
            all_detected[fid] = {
                "id": fid,
                "detector": detector,
                "file": file,
                "tier": int(finding.get("tier", 3) or 3),
                "confidence": finding.get("confidence", "medium"),
                "summary": finding.get("summary", "Ignored finding"),
...
def _upsert_findings(
    ...
    Returns (ids, new, reopened, by_detector, ignored, ignored_by_detector,
             ignored_by_tier, ignored_findings).
    """
    current_ids: set[str] = set()
    new_count = reopened = ignored_count = 0
    by_detector: dict[str, int] = {}
    ignored_by_detector: dict[str, int] = {}
    ignored_by_tier: dict[str, int] = {}
    ...
    for f in current_findings:
        ...
        if is_ignored(fid, f["file"], ignore):
            ignored_count += 1
            det = f.get("detector", "unknown")
            ignored_by_detector[det] = ignored_by_detector.get(det, 0) + 1
            tier = int(f.get("tier", 3) or 3)
            tier_key = f"T{tier}"
            ignored_by_tier[tier_key] = ignored_by_tier.get(tier_key, 0) + 1
...
def merge_scan(state: dict, current_findings: list[dict], *,
    ...
    state["ignore_integrity"] = {
        ...
        "ignored_by_detector": dict(sorted(ignored_by_detector.items())),
        "ignored_by_tier": dict(sorted(ignored_by_tier.items())),

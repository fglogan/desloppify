...
def _show_score_delta(state: dict,
    ...
    if non_comparable_reason:
        overall_delta_str = objective_delta_str = strict_delta_str = ""
        overall_color = objective_color = strict_color = "dim"
    else:
    ...
    if non_comparable_reason:
        print(colorize(f"  Δ non-comparable: {non_comparable_reason}", "yellow"))
...
def cmd_scan(args) -> None:
    ...
    prev_tool_hash = state.get("tool_hash")
    from ..state import get_overall_score, get_objective_score, get_strict_score
    ...
    if path_changed:
        non_comparable_reason = f"scan path changed ({prev_scan_path} -> {scan_path_rel})"
    else:
        current_tool_hash = state.get("tool_hash")
    ...
    _write_query({"command": "scan",
                  ...
                  "delta_comparable": non_comparable_reason is None,
                  "delta_comparable_reason": non_comparable_reason,
                  "profile": profile,
